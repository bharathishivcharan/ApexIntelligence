...
def plot_pro_dashboard(table1, table2, name1, name2):
    ...
    # --- Plot 2: Throttle & Brake ---
    #Top Half: Throttle (0 to 100)
    ax[1].plot(d1['Distance'], d1['Throttle'], color=c1, label=f'{name1} Throttle', linewidth=1.5)
    ax[1].plot(d2['Distance'], d2['Throttle'], color=c2, label=f'{name2} Throttle', linewidth=1.5, linestyle='--')
    #Bottom Half: Brake (0 to -100)
    ax[1].plot(d1['Distance'], d1['Brake'] * -100, color=c1, linewidth=1.5)
    ax[1].plot(d2['Distance'], d2['Brake'] * -100, color=c2, linewidth=1.5, linestyle='--')

    ax[1].axhline(0, color='white', linewidth=0.8, alpha=0.5)
    # Display both
    ax[1].set_yticks([-100, -50, 0, 50, 100])
    ax[1].set_yticklabels(['100% Brake', '50%', '0', '50%', '100% Throttle'])
    ax[1].set_ylim(-110, 110)
    ax[1].set_ylabel('Pedal Input', fontsize=12, fontweight='bold')

    # --- Plot 3: RPM (Replacing Steering) ---
    ax[2].plot(d1['Distance'], d1['RPM'], color=c1, linewidth=1.5)
    ax[2].plot(d2['Distance'], d2['RPM'], color=c2, linewidth=1.5, linestyle=':')
    ax[2].set_ylabel('Engine RPM', fontsize=12, fontweight='bold')
    ...
    plt.savefig('visuals/Leclerc_speed_profile.png', bbox_inches='tight', dpi=300)
    print("Full dashboard saved to visuals/Leclerc_speed_profile.png")
    plt.show()
